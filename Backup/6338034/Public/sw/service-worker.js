...
const relativeUrl   = 'index',
      serverUrl     = self.registration.scope,
...
function handle_push(event) {
    ...
    if (jwt !== undefined) {
        let chapterArray = JSON.parse(event.data.text()),
            url,
            reqArr = [],
            newReq;
        caches.open(version)
        .then(function(cache) {
            ...
            chapterArray.forEach(story => {
                story.chapters.forEach((chapter) => {
                    url = serverUrl.replace('dev/', '') + 'api/requestChapter?short=' + story.short + '&chapter=' + chapter;
...
function displayMessage(data) {
    let chapterArray = JSON.parse(data),
        body = pastBody;
