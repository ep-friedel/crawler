...
admin.serverActions.getSeriesSettings = () => {
    ...
    return new Promise((resolve, reject) => {
        http.open('GET', '/api/requestSeriesSettings?t=' + new Date().getTime(), true);
        http.onreadystatechange = () => {
            if(http.readyState == 4 && http.status == 200) {
...
admin.serverActions.getFullChapterList = () => {
    ...
    return new Promise((resolve, reject) => {
        http.open('GET', '/api/requestFullChapterList?t=' + new Date().getTime(), true);
        http.onreadystatechange = () => {
            if(http.readyState == 4 && http.status == 200) {
...
admin.handler.toggleAddSeries = (event) => {
    ...
    if (type === 'oneVar') {
        ...
        [8,1,2,3,4,5,6,7,10].forEach((cnt) => {
            items[cnt].className = items[cnt].className.replace(/ hideSoft/g, '');
        });
...
admin.handler.addSeries = (edit, event) => {
    ...
    dataString = 'name=' + name + '&short='+ short + '&url1='+ url1 + '&url2='+ url2 + '&url3='+ url3  + '&currentLink='+ currentLink + '&bookChapterReset='+ bookChapterReset + '&rss=' + rss;
...
admin.proto.picker = function(options, cb) {
    ...
    self._setOptions = () =>{
        if (options.target !== undefined || options.data !== undefined){
            ...
            self.options.factor = options.factor || 2;
    ...
    self._mousemove = (event) => {
        ...
        self.currentX = parseInt(event.clientX);
        self.currentY = parseInt(event.clientY);
        var deltaY = self.startY - self.currentY
        ,   deltaX = self.startX - self.currentX
        ,   Xfactor = (deltaX > 0) ? Math.round(deltaX / 10)  + 0.25 : Math.round(-deltaX / 10)  + 0.25
        ,   move = -(deltaY * Xfactor * self.options.factor) + self.startTrans
        ,   smoothMove;

        if (move < 0 && move > -self.list.scrollHeight + self.startTrans + self.list.offsetHeight) {
            smoothMove = Math.round(move / self.list.children[0].getBoundingClientRect().height) * self.list.children[0].getBoundingClientRect().height;
    ...
    self._touchmove = (event) => {
        ...
        self.currentX = parseInt(event.touches[0].clientX);
