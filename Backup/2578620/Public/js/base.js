...
,   el: {
    ...
    ,   readerList: document.querySelector('#reader').children[0]
    ,   message: document.querySelector('#message')
    ,   menu: document.querySelector('#menu')
    ,   console: document.querySelector('#console')
    ,   consoleButton: document.querySelector('#hideConsole')
    ...
    if (ChapterElem !== null) {
        if (dir === 'l' ) {
            if (front.vars.startTrans + front.vars.screenX <= 0 ){
                front.methods.hideItem(ChapterElem);
            ...
            if ((front.vars.screenX - front.vars.startTrans) < cseList.scrollWidth){
                if (!front.methods.hideItem(ChapterElem)) {
                    cseList.style.transform = 'translateX(' + (-front.vars.screenX + front.vars.startTrans)  + 'px)';
                }
            }
        }
...
front.handler.menu.show = (event) => {
    const menu = front.el.menu,
          consoleElem = document.querySelector('.console');
...
front.handler.markReadButton = (event) => {
...
front.handler.toggleSubscription = (event) => {
...
front.handler.toggleConsole = (show) => {
...
front.handler.reload = () => {
    front.handler.toggleConsole(true);
    front.methods.getChapterList()
        .then(() => {
            setTimeout(front.handler.toggleConsole, 1000, false);
        }).catch(err => console.warn('reload error', err));
};

front.handler.fullReload = () => {
    location.reload();
};
...
front.methods.getChapter = (arr, addToNew) => {
    return new Promise((resolve, reject) => {
        let recursiveCrawlingFunction = (index) => {
            let item = arr[index];
            if (typeof(item.short) !== 'undefined' && typeof(item.Chapter) !== 'undefined') {
                ...
                front.serverActions.requestChapter(item, addToNew)
                    ...
                    .then(() => {
                        index++;
                        if (index < arr.length) {
                            recursiveCrawlingFunction(index);
                        } else {
                            resolve();
                        }
...
front.methods.newChapterList = () => {
    if (front.vars.newChapterList.length > 0) {
        ...
        front.methods.getChapter(front.vars.newChapterList)
            .then(() => {
                front.handler.toggleConsole(false);
...
front.methods.refreshedChapterList = () => {
    if (front.el.body.classList.contains('menuMode')) {
        front.vars.refresh = false;
        Array.from(front.el.readerList.children).forEach((el) => {
                el.classList.add('hidden', 'allRead');
        });
        ...
        front.vars.createdStoryElem.forEach(item => item.menu.firstElementChild.removeEventListener('click', front.methods.refreshedChapterList));
    } else {
        front.vars.refresh = true;
        front.vars.createdStoryElem[front.vars.currentStory].menu.firstElementChild.addEventListener('click', front.methods.refreshedChapterList);
    }
};
...
front.methods.renderChapter = (item) => {
    ...
    if (cse[item.short] === undefined) {
        ...
        cse[item.short].mark = document.createElement('div');
        cse[item.short].mark.className = 'markReadButton';
        cse[item.short].mark.appendChild(document.createElement('span'));
        cse[item.short].menu.appendChild(cse[item.short].mark);
        cse[item.short].mark.addEventListener('click', front.handler.markReadButton, false);
        if (front.el.reader.className === 'readerMode'){
            cse[item.short].menu.className += 'hidden';
        }
    }
...
front.createSubscriptionList = (fullChapterList, subscriptionList) => {
    ...
    Promise.all([fullChapterList, subscriptionList]).then(() => {
        ...
        l.addEventListener('click', front.handler.toggleSubscription);
...
front.registerListeners = () => {
    ...
    document.querySelector('#reloadPageButton').addEventListener('click', front.handler.fullReload);
    document.querySelector('#resetStorageButton').addEventListener('click', m.resetStorage);
    document.querySelector('#hideConsole').addEventListener('click', h.toggleConsole);
...
front.initAfterLogin = () => {
    ...
    /*front.checkServerTimer = setInterval(front.methods.getChapterList, 30000);*/
...
window.onresize = () => {
    ...
    front.vars.resizer = setInterval(() => {
        const cse = front.vars.createdStoryElem,
